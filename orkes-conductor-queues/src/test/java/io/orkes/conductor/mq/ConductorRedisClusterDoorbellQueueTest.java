...
/**
 * Runs the full {@link AbstractConductorQueueTest} correctness suite against a real Redis Cluster
 * with the {@link RedisDoorbell} enabled. This is what proves the doorbell is cluster-safe: every
 * doorbell operation is single-key (per-partition list, hash-tagged to one slot), so the ring
 * ({@code LPUSH}/{@code LTRIM}) and the listener {@code BLPOP} run on a cluster without a {@code
 * CROSSSLOT} error. Uses TestContainers, so it always runs.
 */
public class ConductorRedisClusterDoorbellQueueTest extends AbstractConductorQueueTest {

    private static final String queueName = "cluster_doorbell_test";
    ...
    public static void setUp() {
        ...
        doorbell = new RedisDoorbell(jedisCluster, 4); // 4 listener threads on the cluster
        clusterQueue =
